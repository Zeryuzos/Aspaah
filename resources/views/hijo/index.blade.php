...
@section('content')
<a href="hijos/create" class="btn btn-primary"><i class="fas fa-plus"></i> CREAR</a>
<div class="card-body">
    ...
    <table id="example1" class="table table-info table-bordered mt-4 table-hover dataTable dtr-inline" role="grid" aria-describedby="example1_info">
        ...
        <tbody class="text-center">
            ...
            <tr class="odd">
                ...
                <td>{{ $hijo->dni_hijo}}</td>
                <td>{{ $hijo->fn_hijo}}</td>
                <td>{{ $hijo->ginstruccion_hijo}}</td>
                <td class="">
                    <form class="d-flex justify-content-around formulario_eliminar" action="{{route ('hijos.destroy',$hijo->id)}}" method="POST">
                    <a class="btn btn-info" href="/hijos/{{ $hijo->id}}/edit"><i class="fas fa-edit"></i> Editar</a>
                    @csrf
                    @method('DELETE')
